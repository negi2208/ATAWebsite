...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/management/order`,
        {
          params: {
            order_id: search,
            status: statusFilter,
  ...
  // 🔥 UPDATED STATS (Backend se aa rahe)
  const stats = [
    { label: "Total Orders", value: counts?.total || 0, icon: "🛒" },
    { label: "Placed", value: counts?.placed || 0, icon: "⏱️" },
            ...
            {loading ? (
              ...
              orders.map((o, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 border">
                    {o.items?.[0]?.variant?.product?.name || "-"}
                  </td>
